...
The workflow can be run manually with a year, month, and lookback month count. If no year and month are provided, the workflow ends at the current calendar month. The lookback defaults to the three preceding months, so each run regenerates four monthly reports in total.

The scheduled workflow runs daily at 06:15 UTC and regenerates the current month plus the configured preceding-month lookback, so in-progress reports and release/tag updates remain current.
